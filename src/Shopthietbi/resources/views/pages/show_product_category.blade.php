        ...
        'loop': false
            ...
            @foreach ($banner as $banner)
                <div class="home-slide2 banner banner-md-vw">
                    <img style="background-color: #ccc; min-height:290px; max-height:290px;"
                        width="1476"src="{{ URL::to('public/upload/banner/' . $banner->slider_image) }}" alt="slider image">
                ...
                <ol class="breadcrumb">
                    ...
                    @foreach ($category_name as $key => $category_name)
                        ...
                        @if ($category_name != null)
                            <li class="breadcrumb-item active" aria-current="page">{{ $category_name->category_name }}
                            </li>
                        @else
                        @endif
                    @endforeach
                ...
                <div class="col-lg-9 main-content">
                    <nav class="toolbox sticky-header" data-sticky-options="{'mobile': true}">
                        <div class="toolbox-left">
                            ...
                            <form action="{{ URL::to('/danh-muc/' . $category_name->category_slug) }}" method="get">
                                <div class="toolbox-item toolbox-sort">
                                    ...
                                    <!-- End .select-custom -->
                                    <button type="submit"
                                        style="height: 34px; display: flex; align-items: center;margin-left:1.1rem;text-transform: none;"
                                        class="btn btn-outline-dark">Lọc</button>
                    ...
                    <div class="row">
                        ...
                        @foreach ($product_by_category as $key => $product_category)
                            <div class="col-6 col-sm-4">
                                <div class="product-default">
                                    <figure>
                                        <a href="{{ URL::to('/san-pham/' . $product_category->product_slug) }}">
                                            <img src="{{ URL::to('public/upload/' . $product_category->product_image) }}"
                                                style="width: 280px; height: 280px;"alt="product">
                                            <img src="{{ URL::to('public/upload/' . $product_category->product_image) }}"
                                                style="transform: scaleX(-1);width: 280px; height: 280px;" alt="product" />
                                        </a>

                                        {{-- @if ($product_category->product_status == '1')
                                <div class="label-group">
                                    <div class="product-label label-hot"> 
                                        Giảm: {{ round(100 - (($product_category->product_sale_price / $product_category->product_price) * 100)) }}%
                                    </div>
                                </div>
                                @else
                                ...
                                @endif --}}
                                    ...
                                    <div class="product-details">
                                        ...
                                        @if ($product_category->promotion_id != 0)
                                            @php
                                                $active_promotion_new = DB::table('tbl_promotion')

                                                    ->where('promotion_id', $product_category->promotion_id)
                                                    ->get();
                                            @endphp
                                            <div class="price-box">
                                                @foreach ($active_promotion_new as $v_active_promotion)
                                                    @if ($v_active_promotion->promotion_status == 'Có')
                                                        @if ($v_active_promotion->promotion_option == '%')
                                                            <del
                                                                class="old-price">{{ number_format($product_category->product_price) }}</del><br>
                                                            <span style="color:red;"
                                                                class="product-price">{{ number_format(($product_category->product_price * (100 - $v_active_promotion->promotion_price)) / 100) . ' ' . 'VNĐ' }}</span>
                                                        @else
                                                            <del
                                                                class="old-price">{{ number_format($product_category->product_price) }}</del><br>
                                                            <span style="color:red;"
                                                                class="product-price">{{ number_format($product_category->product_price - $v_active_promotion->promotion_price) . ' ' . 'VNĐ' }}</span>
                                                        @endif
                                                    @else
                                                        <del class="old-price"></del><br>
                                                        <span style="color:red;"
                                                            class="product-price">{{ number_format($product_category->product_price) . ' ' . 'VNĐ' }}</span>
                                                    @endif
                                                @endforeach
                                            </div>
                                        @else
                                        ...
                                        <p style="color:#999;font-size: 1.4rem;">Đã bán:
                                            {{ $product_category->quantity_sold }}</p>
                                        ...
                                        <div class="product-action">
                                            ...
                                            <form action="{{ URL::to('/add-cart') }}" method="post">
                                                ...
                                                <input type="hidden" name="cart_product_id"
                                                    value="{{ $product_category->product_id }}"
                                                    class="cart_product_id_{{ $product_category->product_id }}">
                                                <input type="hidden" name="cart_product_name"
                                                    value="{{ $product_category->product_name }}"
                                                    class="cart_product_name_{{ $product_category->product_id }}">
                                                <input type="hidden" name="cart_product_image"
                                                    value="{{ $product_category->product_image }}"
                                                    class="cart_product_image_{{ $product_category->product_id }}">
                                                @if ($product_category->promotion_id != 0)
                                                    @php
                                                        $active_promotion_new = DB::table('tbl_promotion')
                                                        ...
                                                        @if ($v_active_promotion->promotion_status == 'Có')
                                                            @if ($v_active_promotion->promotion_option == '%')
                                                                <input type="hidden" name="cart_product_price"
                                                                    value="{{ ($product_category->product_price * (100 - $v_active_promotion->promotion_price)) / 100 }}"
                                                                    class="cart_product_price_{{ $product_category->product_id }}">
                                                            @else
                                                                <input type="hidden" name="cart_product_price"
                                                                    value="{{ $product_category->product_price - $v_active_promotion->promotion_price }}"
                                                                    class="cart_product_price_{{ $product_category->product_id }}">
                                                            @endif
                                                        @else
                                                            <input type="hidden" name="cart_product_price"
                                                                value="{{ $product_category->product_price }}"
                                                                class="cart_product_price_{{ $product_category->product_id }}">
                                                        @endif
                                                    @endforeach
                                                @else
                                ...
                                <div class="widget-body">
                                    <ul class="cat-list">
                                        @foreach ($category as $key => $category1)
                                            <li>
                                                <a href="{{ URL::to('/danh-muc/' . $category1->category_slug) }}"
                                                    id="category_parent_{{ $category1->category_id }}" role="button"
                                                    aria-expanded="false"
                                                    aria-controls="widget-category-{{ $category1->category_id }}">
                        ...
                        <div class="widget">
                            <h3 class="widget-title">
                                <a data-toggle="collapse" href="#widget-body-3" role="button" aria-expanded="true"
                                    aria-controls="widget-body-2">Thương hiệu</a>
                            </h3>
                                ...
                                <div class="widget-body">
                                    <ul class="cat-list">
                                        @foreach ($brand as $key => $brand1)
                                            <li>
                                                <a href="{{ URL::to('/thuong-hieu/' . $brand1->brand_slug) }}"
                        ...
                        <div class="widget">
                            <h3 class="widget-title">
                                <a data-toggle="collapse" href="#widget-body-4" role="button" aria-expanded="true"
                                    aria-controls="widget-body-3">Lọc theo giá</a>
                            </h3>

                            <div class="collapse show" id="widget-body-4">
                                <div class="widget-body pb-0">
                                    <form id="filter_priceForm" autocomplete="off" method="get" action=""
                                        onsubmit="setActionURL(event)">
                                        ...
                                        <div
                                            ...
                                            <div class="filter-price-text" style="width:100%;">
                                                ...
                                                <input type="text" id="amount"
                                                    style="border:0; color:#f61f1f; font-weight:bold;width:100%;">
